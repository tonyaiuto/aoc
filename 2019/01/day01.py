...
presents to other planets! To accurately calculate his position in
space, safely align his warp drive, and return to Earth in time to save
...
def part1():
  assert fuel_per_module(1969) == 654
  assert fuel_per_module(100756) == 33583
  fuel = 0
  with open('input1.txt', 'r') as inp:
    for line in inp:
      fuel += fuel_per_module(int(line.strip()))
  print('Part 1: ', fuel)
...
total. Then, treat the fuel amount you just calculated as the input mass
and repeat the process, continuing until a fuel requirement is zero or
...
def part2():
  ...
  with open('input1.txt', 'r') as inp:
    for line in inp:
      mass = int(line.strip())
      while mass > 0:
        more_fuel = fuel_per_module(mass)
        fuel += more_fuel
        mass = more_fuel
  print('Part 2: ', fuel)
