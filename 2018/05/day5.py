...
def test_ps(inp, expected):
  got = gen_to_str(polescan(polescan(inp)))
  if expected != got:
    print('Fail on <%s>: expected: <%s>, got: <%s>' % (inp, expected, got))


def reduce(s):
  while True:
    o = gen_to_str(polescan(s))
    if o == s:
      return s
    s = o
...
def part2(s):
  ...
  for c in r1:
    chrs.add(c)
  print('Start len=%d, n_units=%s' % (len(r1), len(chrs)))
  for c in chrs:
    r = reduce(r1.replace(c, '').replace(chr(ord(c) ^ 32), ''))
    print('%d: Remove %s' % (len(r), c))


if __name__ == '__main__':
  test_ps('dabAcCaCBAcCcaDA', 'dabCBAcaDA')
  test_ps('dabAaCBAcCcaDA', 'dabCBAcaDA')
  test_ps('dabCBAcCcaDA', 'dabCBAcaDA')
  test_ps('dabCBAcaDA', 'dabCBAcaDA')
  with open(sys.argv[1]) as inp:
    part2(inp.read().strip())
